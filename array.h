...
namespace structures 
{
	...
	{
		if (this == &other)
			throw std::invalid_argument("Arrays are the same!");
		if (this->size_ != other.size_)
			throw std::invalid_argument("Sizes of arrays do not match!");
	...
	T& Array<T>::operator[](const int index)
	{
		...
		byte* indexAdress = this->vector_->getBytePointer(vectorIndex);
		return *(reinterpret_cast<T*>(indexAdress));
	}
	...
	void Array<T>::copy(const Array<T>& src, const int srcStartIndex, Array<T>& dest, const int destStartIndex, const int length)
	{
		...
		if (length + srcStartIndex > src.size_ || length + destStartIndex > dest.size_)
			throw std::exception("here do i end");
