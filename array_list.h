...
namespace structures
{
	...
	{
		if(index >= size_)
			throw std::exception("end here");
		T output = (*array_)[index];
		if(index < size_ - 1)
	...
	{
		size_ = 0;
		// mÙûeme zmeniù na defaultnu veækosù
	}
	...
	inline void ArrayList<T>::enlarge()
	{
		...
		Array<T> * tmpArray = new Array<T>(array_->size() > 0 ? array_->size() * 2 : 4);
		Array<T>::copy(*array_, 0, *tmpArray, 0, static_cast<int>(array_->size()));
		delete array_;
		array_ = tmpArray;
	}
	...
	{
		return (dynamic_cast<const ArrayListIterator*>(&other))->position_ != position_ ||
			(dynamic_cast<const ArrayListIterator*>(&other))->arrayList_ != arrayList_;
	}
